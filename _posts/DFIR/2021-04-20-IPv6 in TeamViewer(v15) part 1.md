...
이 부분에는 A의 컴퓨터 이름이 남는 것으로만 흔히 알고 있다. 그런데 조금 더 정확하게는 일단 팀뷰어에 로그인 후에 원격 통신을 한 경우와 로그인을 하지 않고 원격 통신을 한 경우로 나눠진다.

대부분의 사람들은 로그인을 하지 않고 사용한다. 만일 로그인을 하지 않고 프로그램을 사용하면, 팀뷰어는 원격 대상자(B)에게 보일 이름으로 **컴퓨터의 이름으로 초기 설정**된다. 그리고 이 값이 로그에 남는 것이다.
