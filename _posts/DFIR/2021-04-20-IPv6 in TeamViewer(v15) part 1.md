...
특이한 점은 프로그램을 재설치 하더라도 값이 변하지 않았다.

### 2. A's Display Name or Teamviewer ID(if login)
이 부분에는 A의 컴퓨터 이름이 남는 것으로만 흔히 알고 있다. 그런데 조금 더 정확하게 판단하기 위해서 일단 팀뷰어에 로그인 후에 원격 접근을 했는지 로그인을 하지 않았는지로 나눠진다.

대부분이 로그인을 하지 않고 사용하기에 로그인을 하지 않을 경우를 살펴보면, 팀뷰어는 원격 대상자(B)에게 보일 이름으로 **컴퓨터의 이름으로 초기 설정**되고 이 값이 로그에 남는 것이다.
...
## Wrap-up
Connections_incoming.txt 파일에서 원격으로 접속한 사람의 정보를 일부 확인할 수 있었다.
1. Teamviewer ID  
  ➔ 팀뷰어에서 자체 생성되는 고유 ID값
2. Display Name  
  ➔ 만일 이름이 PC명이 아니라면 팀뷰어로 로그인한 계정일 가능성 존재
