...
## This Post Covers
지난 글에서 특정 LBA 크기만큼 해시값을 계산할 수 있는 기능을 확인하였다. 그리고 이를 통해 원본(4GB USB) 및 사본 디스크(1TB HDD)의 크기가 서로 다른 상황에서 Drive Trim을 비활성화한 상태로 디스크 복제한 경우(Mirror Clone), 원본 디스크의 LBA 크기만큼만 해시값을 계산하는 옵션을 주어 해시 검증을 직접 해보았다.
